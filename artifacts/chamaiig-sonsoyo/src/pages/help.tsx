...
const HELP_OPTIONS = [
  {
    type: "teacher_chat",
    apiRole: "teacher",
    label: "Багштайгаа ярилцмаар байна",
    emoji: "👨‍🏫",
    gradient: "from-blue-500 to-indigo-400",
    desc: "Багш тантай ярилцахад бэлэн байна",
  },
  {
    type: "psychologist",
    apiRole: "psychologist",
    label: "Сэтгэл зүйчтэй уулзмаар байна",
    emoji: "🧠",
    gradient: "from-violet-500 to-purple-400",
    desc: "Мэргэжлийн дэмжлэг авах",
  },
  {
    type: "social",
    apiRole: "both",
    label: "Надад аюултай санагдаж байна",
    emoji: "🆘",
    gradient: "from-orange-500 to-amber-400",
    desc: "Нийгмийн ажилтанд яаралтай мэдэгдэх",
  },
];
...
export default function Help() {
  ...
  const handleSubmit = async (type: string, label: string, apiRole: string) => {
    if (!studentEmail && !studentPhone) {
      toast({ title: "Имэйл эсвэл утасны дугаараа оруулна уу", variant: "destructive" });
      return;
    }
    setSubmitting(true);
    try {
      const roles = apiRole === "both" ? ["teacher", "psychologist"] : [apiRole];

      for (const role of roles) {
        let targets: string[] = [`${role}@school.mn`];
        try {
          const usersRes = await fetch(`/api/users?role=${role}`);
          const users = await usersRes.json();
          if (Array.isArray(users) && users.length > 0) {
            targets = users.map((u: { email: string }) => u.email);
          }
        } catch {
          // fallback to default
        }
      ...
      toast({
        ...
        description: "Хариуцлагатан тантай удахгүй холбогдох болно.",
      });
    } catch {
  ...
  return (
    <div className="relative pb-12 overflow-hidden">
      ...
      <div className="relative z-10 px-4 md:px-8 max-w-2xl mx-auto space-y-5 mt-2">
        ...
        <div className="rounded-3xl p-6 bg-gradient-to-r from-red-500 to-pink-500 text-white shadow-2xl animate-pulse-glow-red">
          <div className="flex flex-col sm:flex-row items-center gap-5 justify-between">
            ...
            <Button
              ...
              className="w-full sm:w-auto rounded-2xl bg-white text-red-600 hover:bg-red-50 font-black text-2xl px-8 h-14 shadow-lg hover:scale-105 transition-all"
            >
              📞 108
            </Button>
          </div>
        ...
        {/* Help options with inline contact forms */}
        {HELP_OPTIONS.map((item) => (
          <div
            key={item.type}
            className={`card-shimmer rounded-3xl p-5 bg-gradient-to-r ${item.gradient} text-white shadow-xl`}
          >
            <div
              className="flex items-center justify-between gap-4 cursor-pointer"
              onClick={() =>
                !submitted.has(item.type) &&
                setActiveForm(activeForm === item.type ? null : item.type)
              }
            >
              <div className="flex items-center gap-4">
              ...
              {submitted.has(item.type) ? (
                ...
                <Button
                  size="sm"
                  className="rounded-full bg-white/25 hover:bg-white/40 text-white border border-white/30 shrink-0 font-bold"
                >
                  {activeForm === item.type ? "Хаах" : "Хүсэлт"}
                </Button>
              )}
            </div>

            {/* Inline contact form */}
            {activeForm === item.type && !submitted.has(item.type) && (
              <div className="mt-4 border-t border-white/30 pt-4 space-y-3">
                <p className="text-white/90 text-sm font-semibold">
                  Холбоо барих мэдээллээ оруулна уу:
                </p>
                <input
                  className="w-full rounded-2xl px-4 py-2.5 text-sm text-gray-700 outline-none bg-white/90 focus:bg-white transition-colors"
                ...
                />
                <Button
                  className="w-full rounded-2xl bg-white text-gray-800 hover:bg-gray-50 font-bold shadow"
                  onClick={() => handleSubmit(item.type, item.label, item.apiRole)}
                  disabled={submitting}
                >
        ...
        <div className="grid grid-cols-2 gap-3 mt-6">
          {QUOTES.map((q, i) => (
            <div
              key={i}
              className={`card-shimmer rounded-3xl p-4 bg-gradient-to-br ${q.color} text-white shadow-lg`}
        ...
        <div className="rounded-2xl p-4 bg-amber-50 border border-amber-200 text-center mt-4">
          <p className="text-amber-700 text-sm font-semibold">
            ⚠️ Хэрэв та эсвэл таны ойр дотны хүн шуурхай аюулд байвал —{" "}
            <strong>108</strong> эсвэл <strong>103</strong> дугаарт залга!
          </p>
        </div>
      </div>
